...
public class GameOverScreen extends AbstractGameScreen implements Screen {
  ...
  public void drawUsername() {
    game.usernameFont.draw(
        game.batch, usernameBottom, BodyConquest.V_WIDTH / 2.0f - 250.0f, 400.0f);
    game.usernameFont.draw(game.batch, usernameTop, BodyConquest.V_WIDTH / 2.0f - 250.0f, 200.0f);
  }

  public void drawScore() {
    game.usernameFont.draw(
        game.batch, Integer.toString(scoreBottom), BodyConquest.V_WIDTH / 2.0f + 150.0f, 400.0f);
    game.usernameFont.draw(
        game.batch, Integer.toString(scoreTop), BodyConquest.V_WIDTH / 2.0f + 150.0f, 200.0f);
  }
  ...
  public void checkPressed() {
    ...
    if (Gdx.input.justTouched()) {
      if (backBounds.contains(tmp.x, tmp.y)) {
        ...
        dispose();
        if (gameType == GameType.SINGLE_PLAYER) {
          game.setScreen(new MenuScreen(game, usernameBottom));
        } else {
          if (comms.getPlayerType() == PlayerType.PLAYER_BOTTOM) {
            game.setScreen(new MenuScreen(game, usernameBottom));
          } else {
            game.setScreen(new MenuScreen(game, usernameTop));
          }
        }
      }
    }
