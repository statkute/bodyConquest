...
public class ServerLogic extends Thread {
  ...
  private void raceSelectionLogic(String message) {
    ...
    if (message.startsWith(MessageMaker.RACE_HEADER)) {
      ...
      playerType = Assets.PlayerType.decode(encodedPlayerType);

      if (playerType == Assets.PlayerType.PLAYER_TOP) game.setPlayerTop(disease);
      if (playerType == Assets.PlayerType.PLAYER_BOTTOM) game.setPlayerBottom(disease);
  ...
  private void encounterLogic(String message) {
    ...
    if (message.startsWith(MessageMaker.TROOP_SPAWN_HEADER)) {
      ...
      Assets.UnitType unit =
          Assets.UnitType.decode(message.substring(pointer, pointer + Assets.UnitType.getEncodedLength()));
      pointer += Assets.UnitType.getEncodedLength() + 1;

      Assets.PlayerType playerType =
          Assets.PlayerType.decode(message.substring(pointer, pointer + Assets.PlayerType.getEncodedLength()));
      pointer += Assets.PlayerType.getEncodedLength() + 1;

      Assets.Lane lane = Assets.Lane.decode(message.substring(pointer, pointer + Assets.Lane.getEncodedLength()));
      ...
      String encodedLane = message.substring(pointer, pointer + Assets.Lane.getEncodedLength());
      lane = Assets.Lane.decode(encodedLane);
