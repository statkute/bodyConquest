...
public class EncounterScreen implements Screen {
  ...
  public EncounterScreen(BodyConquest game) {
    ...
    // Set up the map image (PROBLEM: reliant on unit bar which we must now assume the size will be identical for every user)
  ...
  public void spawnUnit(UnitType unitType, Lane lane, PlayerType playerType) {
    ...
    if (playerType.equals(PlayerType.BOT_PLAYER)) {
      if (lane == Lane.BOT) {
        unit.setPosition(botLaneBPSpawnX - (unit.getWidth() / 2), botLaneBPSpawnY - (unit.getHeight() / 2));
        botLaneP1.add(unit);
      } else if (lane == Lane.MID) {
        unit.setPosition(midLaneBPSpawnX - (unit.getWidth() / 2), midLaneBPSpawnY - (unit.getHeight() / 2));
        midLaneP1.add(unit);
      } else if (lane == Lane.BOT) {
        unit.setPosition(topLaneBPSpawnX - (unit.getWidth() / 2), topLaneBPSpawnY - (unit.getHeight() / 2));
        topLaneP1.add(unit);
      }
    }
    ...
    if (playerType.equals(PlayerType.TOP_PLAYER)) {
      if (lane == Lane.BOT) {
        unit.setPosition(botLaneTPSpawnX - (unit.getWidth() / 2), botLaneTPSpawnY - (unit.getHeight() / 2));
        botLaneP2.add(unit);
      } else if (lane == Lane.MID) {
        unit.setPosition(midLaneTPSpawnX - (unit.getWidth() / 2), midLaneTPSpawnY - (unit.getHeight() / 2));
        midLaneP2.add(unit);
      } else if (lane == Lane.BOT) {
        unit.setPosition(topLaneTPSpawnX - (unit.getWidth() / 2), topLaneTPSpawnY - (unit.getHeight() / 2));
        topLaneP2.add(unit);
      }
    }
